...
//
// The first round begins with both players drawing the top card of their decks: 9 and 5. Player 1
// has the higher card, so both cards move to the bottom of player 1's deck such that 9 is above 5.
...
pub fn part1(lines: &[String]) -> u32 {
    ...
    let mut player1_deck: Vec<_> = input_parts[0]
        .iter()
        .skip(1)
        .map(|line| line.parse::<u32>().unwrap())
        .collect();
    let mut player2_deck: Vec<_> = input_parts[1]
        .iter()
        .skip(1)
        .map(|line| line.parse::<u32>().unwrap())
        .collect();

    let winner_deck: Vec<u32>;
    loop {
        ...
        let player1_hand = player1_deck.remove(0);
        let player2_hand = player2_deck.remove(0);

        if player1_hand > player2_hand {
            player1_deck.push(player1_hand);
            player1_deck.push(player2_hand);
        } else {
            player2_deck.push(player2_hand);
            player2_deck.push(player1_hand);
        }
    }
    ...
    winner_deck
        ...
        .rev()
        .enumerate()
        .fold(0, |acc, (pos, card)| acc + (pos as u32 + 1) * card)
}
...
// collecting cards by winning the round causes a player to have all of the cards, they win, and
// the game ends.
//
...
pub fn recursive_combat(player1_deck: Vec<u32>, player2_deck: Vec<u32>) -> (bool, Vec<u32>) {
    let mut player1_deck = player1_deck;
    let mut player2_deck = player2_deck;

    let mut player1_deck_history: Vec<Vec<u32>> = Vec::new();
    let mut player2_deck_history: Vec<Vec<u32>> = Vec::new();

    let player1_won_game: bool;
    let winner_deck: Vec<u32>;
    loop {
        if player1_deck.is_empty() {
            player1_won_game = false;
            winner_deck = player2_deck;
            break;
        } else if player2_deck.is_empty() {
            player1_won_game = true;
            winner_deck = player1_deck;
            break;
        }
        ...
        let player1_won = if player1_deck.len() as u32 >= player1_hand
            && player2_deck.len() as u32 >= player2_hand
        {
            // Recursive game
            let player1_recursive_deck = player1_deck[0..player1_hand as usize].to_vec();
            let player2_recursive_deck = player2_deck[0..player2_hand as usize].to_vec();
            let (player1_won, _) = recursive_combat(player1_recursive_deck, player2_recursive_deck);
            player1_won
        } else {
        ...
        if player1_deck_history.contains(&player1_deck)
            || player2_deck_history.contains(&player2_deck)
        {
            player1_won_game = true;
            winner_deck = player1_deck;
            break;
        ...
        player1_deck_history.push(player1_deck.clone());
        player2_deck_history.push(player2_deck.clone());
    }

    (player1_won_game, winner_deck)
}

pub fn part2(lines: &[String]) -> u32 {
    let input_parts: Vec<_> = lines.split(|line| line == "").collect();
    let player1_deck: Vec<_> = input_parts[0]
        .iter()
        .skip(1)
        .map(|line| line.parse::<u32>().unwrap())
        .collect();
    let player2_deck: Vec<_> = input_parts[1]
        .iter()
        .skip(1)
        .map(|line| line.parse::<u32>().unwrap())
        .collect();

    let (_, winner_deck) = recursive_combat(player1_deck, player2_deck);
...
mod tests {
    ...
    #[test]
    pub fn test_play_recursive() {
        assert_eq!(
            recursive_combat(vec![9, 2, 6, 3, 1], vec![5, 8, 4, 7, 10]),
            (false, vec![7, 5, 6, 2, 4, 1, 10, 8, 9, 3])
        )
    }
}
