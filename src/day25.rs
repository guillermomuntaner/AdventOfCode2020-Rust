...
// temporarily out of service, so it takes what little energy you have left to even climb the stair
// s and navigate the halls. You finally reach the door to your room, swipe your card, and - beep -
// the light turns red.
//
...
// number. To transform a subject number, start with the value 1. Then, a number of times called the
// loop size, perform the following steps:
//
...
// determine that the door's loop size is 11, because transforming the initial subject number of 7
...
// calculate the encryption key. Transforming the subject number of 17807724 (the door's public key)
// with a loop size of 8 (the card's loop size) produces the encryption key, 14897079. (Transforming
// the subject number of 5764801 (the card's public key) with a loop size of 11 (the door's loop
// size) produces the same encryption key: 14897079.)
//
// What encryption key is the handshake trying to establish?

pub fn part1(door_public_key: u64, card_public_key: u64) -> u64 {
    fn find_loops(subject: u64, target_key: u64) -> u64 {
        let mut loops = 0;
        let mut value = 1;
        loop {
            if value == target_key {
    ...
    let door_loops = find_loops(7, door_public_key);
    let card_loops = find_loops(7, card_public_key);

    fn encrypt(subject: u64, loops: u64) -> u64 {
        let mut value = 1;
        for _ in 0..loops {
            value *= subject;
            value %= 20201227;
    ...
    let door_encryption_key = encrypt(card_public_key, door_loops);
    let card_encryption_key = encrypt(door_public_key, card_loops);

    assert_eq!(door_encryption_key, card_encryption_key);
